...
export const placeQueries = createQueryKeys("place", {
  ...
  getPhoto: ({ photoId, placeId }: GetPhotoParams) => ({
    queryKey: ["getPhoto", { photoId, placeId }],
    queryFn: () => apiClient.get(`api/places/${placeId}/photos/${photoId}`).json<ApiResponseDTO<PhotoDTO>>(),
  }),
});
