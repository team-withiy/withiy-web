...
vi.mock("@/entities/user/api/user.queries", () => ({
  userQueries: {
    getMe: {
      queryKey: ["getMe"],
      queryFn: vi.fn(() => ({
        data: { data: mockUserWithoutRegistered },
        refetch: vi.fn(() => Promise.resolve({ data: mockUserWithoutRegistered })),
      })),
    },
  },
}));
...
test("프로필 페이지가 올바르게 렌더링되어야 한다", async () => {
  renderWithProviders(<ProfilePage termAgreements={mockTermAgreements} onClickPrev={mockOnClickPrev} />);
...
test("닉네임을 입력하지 않으면 버튼이 비활성화되어야 한다", async () => {
  renderWithProviders(<ProfilePage termAgreements={mockTermAgreements} onClickPrev={mockOnClickPrev} />);
