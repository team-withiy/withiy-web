...
test("suspenseQuery 결과가 children에게 전달되어야 한다", async () => {
  renderWithProviders(
    <QueryBoundary queries={[query1]}>{([{ data }]) => <div data-testid="query1-result">{data}</div>}</QueryBoundary>,
  );
...
test("여러 개의 suspenseQuery 결과가 children에게 전달되어야 한다", async () => {
  renderWithProviders(
    <QueryBoundary queries={[query1, query2]}>
      {([result1, result2]) => (
...
test("combine 함수를 사용하여 결과를 조합할 수 있어야 한다", async () => {
  renderWithProviders(
    <QueryBoundary queries={[query1, query2]} combine={(results) => results.map((result) => result.data).join(", ")}>
...
test("오류가 발생한 query는 ErrorBoundary에 의해 처리되어야 한다", async () => {
  renderWithProviders(
    <ErrorBoundary fallbackRender={({ error }) => <div data-testid="error-message">Error: {error.message}</div>}>
      <QueryBoundary queries={[query1, query3]}>
        {([result1, result2]) => (
          <div data-testid="combined-result">
  ...
  await waitFor(() => expect(screen.queryByTestId("combined-result")).not.toBeInTheDocument());
